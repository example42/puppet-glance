...
# = Define: glance::conf
#
# With this define you can manage any glance configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/glance/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/glance/my.conf',
#
# [*ensure*]
...
define glance::conf (
  ...
  $manage_require = pickx($require, $glance::config_file_require)
  $manage_notify  = pickx($notify, $glance::manage_config_file_notify)
  $manage_replace = pickx($replace, $glance::config_file_replace)

  file { "glance_conf_${name}":
    ensure  => $ensure,
    source  => $source,
